...
const paymentHtml = `
    ...
    <div class="max-w-7xl mx-auto px-8 pb-12">
        <div class="flex flex-col items-center gap-4 pt-8 border-t border-gray-100 dark:border-gray-800">
            ...
            <div class="flex flex-wrap justify-center items-center gap-x-8 gap-y-4 opacity-50 grayscale hover:grayscale-0 hover:opacity-100 transition-all duration-700">
                ${visaSvg}
                ${mastercardSvg}
                ${paypalSvg}
                <div class="flex items-center gap-1.5">
                    <span class="material-symbols-outlined text-[20px]">account_balance</span>
                    <span class="text-[10px] font-bold text-gray-500 uppercase tracking-tight">Transferencia</span>
                </div>
                <div class="flex items-center gap-1.5">
                    <span class="material-symbols-outlined text-[20px]">payments</span>
                    <span class="text-[10px] font-bold text-gray-500 uppercase tracking-tight">Efectivo</span>
                </div>
            </div>
        </div>
    </div>
...
files.forEach(f => {
    ...
    if (c.includes('<!-- Payment Methods -->')) {
        ...
        console.log(`Resized SVG fix for ${f}`);
    }
});
